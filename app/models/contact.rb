# A {Contact} is an ordered pair of {User Actors},
...
class Contact < ActiveRecord::Base
  ...
  scope :sent_by, lambda { |a|
    where(:sender_id => User.normalize_id(a))
  }

  scope :received_by, lambda { |a|
    where(:receiver_id => User.normalize_id(a))
  }

  scope :sent_or_received_by, lambda { |a|
    where(arel_table[:sender_id].eq(User.normalize_id(a)).
          or(arel_table[:receiver_id].eq(User.normalize_id(a))))
  }
  ...
  scope :index, lambda { |params|
    in_direction_with(params[:subject], params[:d]).
      positive.
      merge(User.subject_type(params[:type])).
      merge(User.name_search(params[:q])).
  ...
  #
  # Defaults to the sender User, if it is a user
  def user_author
    @user_author ||
      build_user_author
  end
  ...
  def user_author= subject
    @user_author = (subject.nil? ? nil : User.normalize(subject))
  end
